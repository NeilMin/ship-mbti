...
**两处按推荐执行、用户可否决的假设：**
- A1 标题体系 = 单名人设 + 假正经副标（而非保留双段式标题）。
- A2 稀有度 = SSR/SR/R/N 抽卡档位（而非编百分比/不做稀有度）。
...
| Code | 主名 | 副标（假正经职称） | 稀有度 |
|---|---|---|---|
| COLG | 赛博炼丹师 | Agentic Workflow 布道师 | SSR |
| COLW | Design Doc 战神 | 跨组赋能 · 平台化架构负责人 | R |
| COPG | 风口冲浪手 | 全周期创新孵化专家 | SR |
| COPW | 养老院架构师 | 长期技术愿景负责人 | SR |
| CALG | MVP 缝合怪 | 0-1 全栈增长工程师 | R |
| CALW | ROI 做题家 | 结果导向交付负责人 | N |
| CAPG | 「您说的对」工程师 | 人机协同结对编程先锋 | R |
| CAPW | Ticket 消消乐 | 资深敏捷交付专家 | N |
| TOLG | 古法编程仙人 | 代码质量委员会主席 | SSR |
| TOLW | 护城河总工 | 核心系统唯一负责人 | SR |
| TOPG | 手冲代码大师 | 工程美学与最佳实践守护者 | SSR |
| TOPW | 祖传代码守墓人 | 核心系统稳定性负责人 | SR |
| TALG | 键盘特种兵 | 疑难杂症攻坚小组组长 | SSR |
| TALW | 交付牛马 | 核心业务基石工程师 | N |
| TAPG | Stack Overflow 考古家 | 开源社区资源整合专家 | R |
| TAPW | 精神离职者 | 存量系统维护专家 | N |

稀有度编辑逻辑（不是统计，是官方钦定的社会学玩笑）：SSR 给 4 个理想主义浓度最高的 G 型——呼应真实素材"真正热爱钻研技术的是极少数极少数"；N 给 4 个最人均的 W 型（"量产型"本身就是 roast）；其余按稀缺感分 SR/R。每档配一句钦定文案：
- SSR："官方钦定保护动物。"
- SR："传说中见过。"
- R："每层楼都有一个。"
- N："量产型。你组里至少三个。"
...
```ts
export type RarityTier = "SSR" | "SR" | "R" | "N";
...
export interface Personality {
  ...
  rarity: { tier: RarityTier; note: string }; // 新
}
```

`AppCopy.resultSections` 新增 `nemesis` / `soulmate` / `rarity` 三个 label。

### 4.2 组件

- **ResultScreen**：hero 区 title 下渲染 subtitle；code 旁渲染 rarity 徽章（`SSR` 等字样，note 用徽章旁小字，海报与结果页同款，不依赖 hover）；result-grid 新增"天敌 / 灵魂搭子"卡片（对方主名 + note）。
- **ShareCard**（海报是传播物，钩子必须上）：header 加 rarity 徽章与 subtitle；sections 间加一行"天敌：XXX ｜ 搭子：XXX"。
- **样式**：app.css 加徽章（四档四色）与关系卡样式，保持现有设计语言。

### 4.3 测试

- `personalities.test.ts` 新增：每型 subtitle 非空；nemesis/soulmate code 合法且 ≠ 自身；nemesis 满足四字母全反；rarity.tier ∈ {SSR,SR,R,N}。
...
- heroNote → **`AI 依赖、架构洁癖、排错玄学、上班动机——四个维度交叉出 16 种码农。测完拿到你的行为观察报告、你的天敌、你的灵魂搭子，和官方钦定稀有度。`**
...
结构（每型）：① 场景开场把人钉死 → ② 两句处刑（截图点）→ ③ 结尾立场反转（先扎再抱）。strengths 写成"公司留你的理由"口吻；risks 写成"事故预告"口吻；lifestyle 必含一个真实 milieu 细节（Costco/Tahoe/狼人杀/相亲现场/Patagonia）。

### 样稿 1：CAPW · Ticket 消消乐（N）
...
- **稀有度**：N·量产型。你组里至少三个，其中一个正在看这行字。

### 样稿 2：COLW · Design Doc 战神（R）
...
梗库工作文件（本机 scratchpad `genglib.md`，核心条目已内化进本文档）。来源：一亩三分地（刻板印象帖 736103、摆烂 PIP 帖 1133763、地典黑话帖 933587、相亲分析帖 1116544）、huaren.us 婚恋市场帖、zhaoolee/xiaohongshu-programmer-memes、腾讯云"会 Vibe Coding 的同事"、卡思数据 MBTI 传播分析、芝加哥时报硅谷相亲群卧底文。
